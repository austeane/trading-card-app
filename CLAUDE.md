...
# Development - requires --mode mono for non-interactive terminals
...
Tournament configs are stored **only in S3** and managed via the admin UI. There are no hardcoded tournament configs in the codebase.

### Admin UI

Access the admin panel to manage tournaments:
- Create new tournaments
- Edit card types, teams, positions
- Upload logos (team, tournament, org)
- Publish/unpublish tournaments
- Import/export tournament bundles (ZIP)

### Direct S3 Access

For quick config changes without the admin UI:

**View production config:**
\`\`\`bash
# List tournaments
AWS_PROFILE=prod aws s3 cp s3://trading-card-app-production-mediabucket-mdntnkkv/config/tournaments/list.json -

# View specific tournament config
AWS_PROFILE=prod aws s3 cp s3://trading-card-app-production-mediabucket-mdntnkkv/config/tournaments/<tournament-id>/published/config.json - | jq

# View just cardTypes
AWS_PROFILE=prod aws s3 cp s3://trading-card-app-production-mediabucket-mdntnkkv/config/tournaments/<tournament-id>/published/config.json - | jq '.cardTypes'
\`\`\`

**Edit and upload config:**
\`\`\`bash
# Download, edit, and re-upload
AWS_PROFILE=prod aws s3 cp s3://trading-card-app-production-mediabucket-mdntnkkv/config/tournaments/<tournament-id>/published/config.json /tmp/config.json
# Edit /tmp/config.json
AWS_PROFILE=prod aws s3 cp /tmp/config.json s3://trading-card-app-production-mediabucket-mdntnkkv/config/tournaments/<tournament-id>/published/config.json --content-type "application/json" --cache-control "public, max-age=60"
\`\`\`

**Dev stage bucket:** \`trading-card-app-austin-mediabucket-*\`

### Config Structure

Tournament configs (\`TournamentConfig\` type) include:
- \`id\`, \`name\`, \`year\` - Basic info
- \`branding\` - Logo keys, primary color, event indicator
- \`teams[]\` - Team id, name, logoKey
- \`cardTypes[]\` - Type, enabled flag, label, positions, logoOverrideKey
- \`templates[]\` - Template id, label, layout
- \`defaultTemplates\` - Fallback template mapping

### Layout Constants

Visual template layouts (font sizes, positions, colors) are defined in \`shared/src/constants.ts\` as \`USQC26_LAYOUT_V1\`. These define HOW cards render, not tournament-specific data.
...
6. **SST Dev Mode** - Use \`--mode mono\` flag when running \`sst dev\` in non-interactive terminals (CI, scripts, Claude Code). This disables the TUI that requires a TTY.
